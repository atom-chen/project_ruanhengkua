...
int CListCtrlEx::FindItem(LVFINDINFO* pFindInfo, int nStart ) const
{
	if (pFindInfo->flags & LVIF_PARAM)
	{
		...
		{
			if (pFindInfo->lParam == GetItemData(i))
				return i;
		}
		return -1;
	}
...
int CALLBACK CListCtrlEx::CompareProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	if (!(This->m_nSortColumn < 0 || This->m_nSortColumn >= This->GetColumnCount()))
	{
		...
		if (pColInfo && (pColInfo->m_eSort & SortBits)) 
		{
			...
			if (!This->m_fnCompare && pColInfo->m_fnCompare)
			{
				This->m_fnCompare = pColInfo->m_fnCompare;
			}
		}
	}
	...
	if (This->m_fnCompare && This->m_fnCompare != &CListCtrlEx::CompareProc) 
	{
		...
		if (pD1)
		{	
			lParam1 = pD1->m_dwUserData;
		}
		if (pD2)
		{
			lParam2 = pD2->m_dwUserData;
		}
	...
	if (nLeft < 0 || nRight < 0 || nLeft >= nCount || nRight >= nCount)
		return 0;
...
BOOL CListCtrlEx::SortOnColumn(int nColumn, BOOL bChangeOrder)
{
	...
	if ((pColInfo = GetColumnInfo(nColumn)) && (pColInfo->m_eSort & SortBits ))
	{
		if (pColInfo->m_eSort & Auto)
		{
			...
			if (bChangeOrder)
			{
				pColInfo->m_eSort = (Sort)(pColInfo->m_eSort ^ (Ascending | Descending));
			}
		}
...
BOOL CListCtrlEx::DisplayEditor(int nItem, int nSubItem)
{
	...
	if (nItem < 0 || nItem > nCount || nSubItem < 0 || nSubItem > this->GetColumnCount() || IsColumnReadOnly(nSubItem) || IsRowReadOnly(nItem) || IsCellReadOnly(nItem, nSubItem))
		return FALSE;
	...
	if (!m_pEditor->m_pfnInitEditor)
	{
		m_pEditor->m_pWnd->SetWindowText(text);
	}
...
CListCtrlEx::CellInfo* CListCtrlEx::GetCellInfo(int nItem, int nSubItem)
{
	...
	for (INT_PTR i = 0; i < nCount; i++)
	{
		...
		if (pInfo && pInfo->m_nColumn == nSubItem)
			return pInfo;
	}
	return NULL;
}
...
void CListCtrlEx::HideEditor(BOOL bUpdate)
{
	...
	{	
		if (m_pEditor && m_pEditor->m_pWnd)
		{
			...
			if (GetItemCount() > m_nEditingRow)
			{
				Update(m_nEditingRow);
			}
			if (bUpdate == -1)
			{
				SortOnColumn(m_nEditingColumn);
			}
			m_pEditor = NULL;
		}
	}
...
{
	if ((m_hAccel && GetParent() && GetFocus() == this && ::TranslateAccelerator(GetParent()->m_hWnd, m_hAccel, pMsg)))
		return TRUE;
...
{
	if (nRow < 0 || nRow >= GetItemCount() || nColumn < 0 || nColumn >= GetColumnCount())
		return;
...
void CListCtrlEx::SetColumnReadOnly(int nColumn, bool bReadOnly)
{
	...
	if (!pInfo)
		SetColumnEditor(nColumn, (CWnd*)NULL);
...
{
	if (nColumn < 0 || nColumn >= GetColumnCount() || !(eSort & SortBits))
		return;
...
void CListCtrlEx::SetCellReadOnly(int nRow, int nColumn, bool bReadOnly )
{
	...
	if (!pInfo)
	{
		SetCellEditor(nRow, nColumn, (CWnd*)NULL);
	}
...
int CListCtrlEx::Compare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	if (!This || This->m_nSortColumn < 0 || This->m_nSortColumn >= This->GetColumnCount())
		return 0;
...
int CListCtrlEx::CompareNumberString(LPCSTR pLeftText, LPCSTR pRightText)
{
	...
	if (l1 && l2 && (l1 - l2))
	{
		...
		if (str1.GetLength() == left.GetLength() && str2.GetLength() == right.GetLength())
			return (int) (l1 - l2);		
	}
	return CString(pLeftText).Compare(pRightText);
}
