 ...
 * Default constructor.
 */
Test_CreationClass::Test_CreationClass(const std::vector<std::string>& titleList, const std::string& key, const std::string& value/*, const char* filename*/) /*: mFile(filename)*/ {
	// 创建明细表解析器,注意:false表示标题在表的底部,反之为true
	mTableParser = new TableParser(titleList, key, value, false);
}
...
const std::vector<Table*>&  Test_CreationClass::createTable(void)
{
	// 对标题进行归类
	...
	// 填充表格数据
	writeTable();
	return mTableParser->getTableList();
}
