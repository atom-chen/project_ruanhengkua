...
public:
	...
	Cell getCell(size_t index);	// 获取索引处的单元格
...
{
public:
	TableParser(const std::vector<std::string>& titleList, const std::string& key, const std::string& value, bool headerAtTop = true);
	~TableParser();
...
private:
	void trimNotFitFromTable(Table *tb);	// 删除表里不符合解析格式的行

private:
	std::vector<std::string> mTitleList;	// 所要解析的明细表的标题头内容
	std::string mKey;	// 关键字
	std::string mValue;	// 统计值
